...
class TikTokDownloader(QObject):
    ...
    api_error_signal = pyqtSignal(str, str)  # Thêm signal mới: url, error_message
    ...
    def __init__(self):
        ...
        # Kiểm tra phiên bản yt-dlp khi khởi tạo
    ...
    def get_video_info(self, url):
        """Get video information from URL with improved error handling"""
        try:
            info = VideoInfo()
            info.url = url
                    ...
                    try:
                        ...
                        try:
                            ...
                            if "private" in error_msg:
                                info.title = "Error: DIALOG_VIDEO_PRIVATE"
                            elif "not available" in error_msg:
                                info.title = "Error: DIALOG_VIDEO_UNAVAILABLE"
                            elif "copyright" in error_msg:
                                info.title = "Error: DIALOG_VIDEO_COPYRIGHT"
                            elif "region" in error_msg:
                                info.title = "Error: DIALOG_VIDEO_REGION_RESTRICTED"
                            elif "api" in error_msg or "tiktok" in error_msg or "extractor" in error_msg:
                                info.title = "Error: DIALOG_TIKTOK_API_CHANGED"
                                # Emit special API error signal
                                self.api_error_signal.emit(url, "TikTok API may have changed. Please update the application.")
                            else:
        ...
        try:
            ...
            if is_audio_request:
                ...
                try:
                    ...
                    # Thêm creationflags=0x08000000 để ẩn cửa sổ cmd (chỉ hoạt động trên Windows)
                    if os.name == 'nt':  # Windows
                        result = subprocess.run(command, capture_output=True, text=True, encoding='utf-8', 
                                               errors='ignore', creationflags=0x08000000)  # 0x08000000 = CREATE_NO_WINDOW
                    else:  # macOS, Linux
